...
var companyinfo = require('./company')

mongoose.connect(process.env.URL,{ useCreateIndex: true,useNewUrlParser: true, useFindAndModify: false ,useUnifiedTopology: true  },(err,data)=>
{   
        if(err)
	{
	  console.log("mongo error",err)
        ...
        app.get('/getbillinfo/:id',(req,res)=>
        {
            billschema.find({id:req.params.id}).then(resp=>
                {
                    Schema.find({id:req.params.id}).then(respdata=>
                        {
                    
                    if(resp.length > 0)
                    {
                        res.status(200).json({status:true,data:resp,buyername:respdata[0]['Name']})                    }
